...
# Test 3: Test middleware example server
echo -e "\n${CYAN}Test 3: Testing x402 middleware example${NC}"
EXAMPLE_SERVER=Core-Blockchain/examples/x402-middleware-server.js
if [ -f "$EXAMPLE_SERVER" ]; then
    node $EXAMPLE_SERVER >/dev/null 2>&1 &
    MIDDLEWARE_PID=$!
    sleep 3

    # Test free endpoint
    if curl -s http://localhost:3000/api/free | grep -qi "free"; then
    ...
    echo -e "${ORANGE}⚠️  Middleware example not found; skipping${NC}"
fi
