...
startValidator(){
  ...
  while [[ $i -le $totalValidator ]]; do
    ...
    if tmux has-session -t node$i > /dev/null 2>&1; then
        ...
        tmux new-session -d -s node$i
        tmux send-keys -t node$i "./node_src/build/bin/geth --datadir ./chaindata/node$i --networkid $CHAINID --bootnodes $BOOTNODE --mine --port 30303 --nat extip:$IP --gpo.percentile 0 --gpo.maxprice 100 --gpo.ignoreprice 0 --miner.gaslimit 20000000000 --unlock 0 --password ./chaindata/node$i/pass.txt --syncmode=snap --gcmode=full --cache=512 --cache.database=256 --cache.trie=128 --cache.gc=128 --txpool.accountslots=10000 --txpool.globalslots=200000 --txpool.accountqueue=10000 --txpool.globalqueue=100000 --verbosity=3 console" Enter
    fi
